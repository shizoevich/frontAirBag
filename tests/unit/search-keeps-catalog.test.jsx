 ...
 *
 * Раньше `/search` был отдельной страницей со своим видом: только сетка товаров.
 * Введённый запрос убирал баннер и все уровни категорий разом, и сузить выдачу было
 * нечем — приходилось возвращаться на главную и начинать заново. Маршрута больше нет,
 * запрос живёт параметром на самой витрине.
 *
 * Теперь запрос — это состояние той же витрины: категории на месте, запрос уходит
 * в тот же запрос к API и виден на странице.
 *
 * Запрос и категория исключают друг друга, побеждает последнее действие: выбор
 * категории снимает запрос, новый запрос уводит из категории. Сортировка и цена —
 * настоящие фильтры и переживают оба.
 */
...
describe('витрина с поисковым запросом', () => {
  ...
  it('выбор категории снимает запрос — они исключают друг друга', () => {
  ...
  it('фильтры выбор категории переживают — они сужают, а не заменяют', () => {
    const { getByText } = renderWithQuery('searchText=подушка&ordering=price');

    getByText('Подушки безпеки').click();

    expect(pushed[0]).toContain('ordering=price');
  });
